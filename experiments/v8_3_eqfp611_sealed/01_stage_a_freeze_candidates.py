...
def main(input_dir, out_dir):
    ...
    diagnostics = v81.model_diagnostics(real_model)
    triplet_entries = int(diagnostics["triplet_crossfit"]["entries"])
    quartet_entries = int(diagnostics["quartet_crossfit"]["entries"])
    if triplet_entries < 1 or quartet_entries < 1:
        qualification = {
            "status": "ABORT_BEFORE_REVEAL",
            "reason": "Higher-order support qualification gate failed.",
            "triplet_entries": triplet_entries,
            "quartet_entries": quartet_entries,
        }
    ...
    stage_a = {
        "version": "NABU_V8_3_EQFP611_STAGE_A_FREEZE_V1",
