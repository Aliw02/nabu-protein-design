...
def run(source_gz: Path, output_dir: Path) -> dict:
    ...
    observed_counts = {
        "V8_3_ADAPTIVE_ROUTER": int(
            source_counts.get("V8_3_ADAPTIVE_ROUTER", 0)
        ),
        "B2_ADDITIVE": int(
            source_counts.get("B2_ADDITIVE", 0)
        ),
        "GLOBAL_MEAN_ABSTENTION": int(
            source_counts.get("GLOBAL_MEAN_ABSTENTION", 0)
        ),
    }
    expected_counts = {
        "V8_3_ADAPTIVE_ROUTER": 131,
        "B2_ADDITIVE": 3517,
        "GLOBAL_MEAN_ABSTENTION": 530,
    }
    if observed_counts != expected_counts:
        raise RuntimeError(
            "Runtime evidence-level counts disagree with the "
            f"label-blind freeze: observed={observed_counts}, "
