...
def run(source_gz: Path, output_dir: Path) -> dict:
    ...
    expected_identity = {
        "fit_count": 3746,
        "validation_count": 662,
        "test_count": 4178,
        "scoreable_count": 131,
        "unscoreable_count": 4047,
    }
    for key, expected in expected_identity.items():
        actual = int(identity_gate[key])
        if actual != expected:
            raise RuntimeError(
                f"IRED identity preflight drift for {key}: "
    ...
    runtime_scoreable = scored["scoreable"].to_numpy(dtype=bool)
    runtime_scoreable_count = int(runtime_scoreable.sum())
    if runtime_scoreable_count != int(identity_gate["scoreable_count"]):
        raise RuntimeError(
            "Runtime scoreability disagrees with identity-only preflight: "
            f"{runtime_scoreable_count} vs {identity_gate['scoreable_count']}"
        )
    ...
    scoreable_target = target[runtime_scoreable]
    scoreable_prediction = prediction[runtime_scoreable]
    scoreable_diagnostic = {
        "count": runtime_scoreable_count,
        "spearman": float(
            spearmanr(scoreable_target, scoreable_prediction).statistic
        ),
        "ndcg": float(
            ndcg_score(
                (scoreable_target - float(np.min(scoreable_target)))[None, :],
                scoreable_prediction[None, :],
            )
        ),
    }
    ...
    predictions = pd.DataFrame(
        {
            ...
            "scoreable": runtime_scoreable,
            "router_score_before_abstention": router_raw,
        }
    )
    ...
    manifest = {
        "version": "NABU_PHASE2D_IRED_BLIND_V2",
        ...
        "abstention_policy": {
            "scoreable_count": runtime_scoreable_count,
            "unscoreable_count": int((~runtime_scoreable).sum()),
            "abstention_floor": abstention_floor,
            "rule": "unscoreable rows tie below all scoreable frozen-router rows",
            "b2_fallback_used": False,
        },
        ...
        "scoreable_only_diagnostic_not_primary": scoreable_diagnostic,
        "secondary_metrics": {
            key: metrics[key]
            for key in (
                "top1_percent_k",
